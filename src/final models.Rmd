...
m_spl_org.fit = fitted(m_spl_arma_org)
...
```
## varx log not avaliable because y needs two variables
```{r}
##Model Selection
VARselect(cbind(ts_hosr_log[1:153]), lag.max = 20,exogen=data.frame(ts_icsa_log[1:153]))$selection

```

```{r}
## Model Fitting: Unrestricted VAR
m_varx_org=VAR(data.frame(hosr=ts_hosr_log[1:153]), p=4,exogen=data.frame(icsa=ts_mspus_log[1:153]))
summary(m_varx_org)
